function [ output ] = decode_linearclassifierLOO ( signal, t, trials, params)
% % decode_linearclassifierLOO%
%PURPOSE:   Decoding with linear classifier (leave one out)
%AUTHORS:   AC Kwan 180727
%
%INPUT ARGUMENTS
...
for n = 0:1:params.nBack
    ...
    for jj=1:numel(output.decode_time)  % each time step
        disp(['Linear classifier LOO - time step ' int2str(jj) '/' int2str(numel(output.decode_time)) ]);
        ...
        for ll=1:numel(event_traintest)
            
            %use this subset of trials to construct classifier
            drawIndex=true(numel(event_traintest),1);
            drawIndex(ll)=false;  %leave one out
            ...
            for j=1:numAddTest
                ...
                testsig_addTest=tempsig_addTest{j}(includeIdx,:);   %no need to leave one out, use all the include-set data to test classifier
